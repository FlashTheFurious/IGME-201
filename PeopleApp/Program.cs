                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                foreach (string courseCode in student.courseCodes)
                                {
                                    ...
                                    if (course != null)
                                    {
                                        ...
                                        Console.WriteLine("{0:hh:mmtt}", course.schedule.startTime);
                                        Console.WriteLine("{0:hh:mmtt}", course.schedule.endTime);
                                        foreach (DayOfWeek days in course.schedule.daysOfWeek)
                                        {
        ...
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                do
                {
                    ...
                    if (newCode.Length > 0)
                    {
                        if (courses[newCode] == null)
                        {
                            Console.WriteLine("Invalid Code");
                            continue;
                        }
                        thisStudent.courseCodes.Add(newCode);
                    }
